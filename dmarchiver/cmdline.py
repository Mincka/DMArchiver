...
def main():
    ...
    password = getpass.getpass('Enter your password (characters will not be displayed): ')
    ...
    if args.conversation_id != None:
        # Prevent error when using '' instead of ""
        conversation_id = args.conversation_id.strip('\'')
        print('Conversation ID specified ({0}). Retrieving only one thread.'.format(args.conversation_id))
        crawler.crawl(conversation_id, args.download_images, args.download_gifs)
    else:
